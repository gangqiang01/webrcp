	...
	if r.Method == "POST" {
		...
		switch submit{
			...
			case "SendByGolang":			//topic:  xxxxxxxxxx/xxxxxxxxxxx/xxxxxxxxxxx/
				...
				log.Println("## jindebug: ", r.Form.Get("msg"))
				log.Println("## gangqiang: ", r.Form.Get("topic"))
				for iter.Next(&result) {
				...
				for iter.Next(&result) {
					if COMPANY == "Guest"{
						...
						if NAME == "admin"{
							...
							iter1 := mongo.Select(acollection, bson.M{"company":COMPANY,"username":NAME}).Iter()
							for iter1.Next(&result1) {
								s:= CheckDeviceStatus(result1.TIME)
								result1.STATUS = s
								result1.GROUP = GetGroupName(result.GROUPS, result1.DEVICEID)
			...
			case "DeviceBindUser":
				...
				selector :=bson.M{"deviceid":DEVICEID}
				data := bson.M{"$set": bson.M{"username": NAME}}
				err := mongo.Update(acollection, selector, data)
				if err != nil {
					panic(err)
				}else{
...
func smtpmail(email string, UserCompany string, UserName string) string{
	...
	//send mail
	from := mail.Address{"", "aim_noreply@163.com"}
    to   := mail.Address{"", email}
    subj := "Subject: Welcome AIMoblie Industrial Android ,Please verify your email address !"
    body := "Hello：\r\n" +
	...
	auth := smtp.PlainAuth("","aim_noreply@163.com", "asdfgh123", host)
    ...
    c.Quit()
	fmt.Println("Sendmail return: ", VerifyStatus)	
	return VerifyStatus
}
...
func smtpresetpass(email string, UserCompany string, UserName string) string{
	...
	from := mail.Address{"", "aim_noreply@163.com"}
    to   := mail.Address{"", email}
    subj := "Subject: Welcome AIMoblie Industrial Android ,Password Reset !"
    body := "Hello：\r\n" +
...
func VerifyEmail(email string, UserCompany string, UserName string)string{
	...
	from := mail.Address{"", "aim_noreply@163.com"}
    to   := mail.Address{"", email}
    subj := "Subject: Welcome AIMoblie Industrial Android ,Chenge email !"
    body := "Hello：\r\n" +
